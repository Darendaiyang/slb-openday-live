...
每条岗位建议包含这些字段（最少前 4 个）：

- `id`：唯一标识（如 `data-intern`）
- `title`：岗位名称
- `city`：城市
- `jd`：岗位描述
- `keywords`：可选关键词数组（用于专业匹配更准）

> 如果你改用 Node 完整版（`npm start`），可以在 `public/admin-jobs.html` 页面维护 JD（需 `ADMIN_TOKEN`）。

---

## 六、背景图自定义（可选）

当前样式支持“图片优先背景”。你有两种方式：

1. 直接改样式变量 `styles.css`：  
   `--custom-bg-image: url("./assets/your-bg.jpg");`
2. 临时通过 URL 指定（会记住）：  
   `https://你的域名/?bg=https://你的图片地址.jpg`
